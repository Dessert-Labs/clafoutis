...
describe('validateTokens', () => {
  ...
  describe('string tokens', () => {
    it('accepts CSS timing function strings like steps() and linear()', () => {
      const files = makeFiles({
        'tokens.json': {
    ...
    it('rejects non-string values for string tokens', () => {
      const files = makeFiles({
        'tokens.json': {
          bad: { $type: 'string', $value: 1234 },
        },
      });
      const results = validateTokens(files);
      expect(results.some((r) => r.code === 'INVALID_VALUE')).toBe(true);
    ...
    const files = makeFiles({
      'motion.json': {
        ...
        easing: {
          ...
          stepStart:  { $type: 'string', $value: 'steps(1, start)' },
          stepFine:   { $type: 'string', $value: 'steps(12, end)' },
          multiStop:  { $type: 'string', $value: 'linear(0, 0.08 12%, 0.32 38%, 0.68 62%, 0.92 88%, 1)' },
        },
      },
    });
    expect(validateTokens(files)).toHaveLength(0);
